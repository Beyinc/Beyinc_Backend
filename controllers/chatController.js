  ...
  try {
    ...
    if (conversationExists.length == 0) {
      ...
      await Conversation.create({
        ...
        lastMessageTo: req.body.receiverId,
        seen: "",
        requestedTo: req.body.receiverId,
        status: "pending",
        pitchId: pitchId,
      });


      if (!receiverInfo.followers.includes(senderInfo._id)) {
        receiverInfo.followers.push(senderInfo._id);
        await receiverInfo.save();
        senderInfo.following.push(receiverInfo._id);
        await senderInfo.save();
       
        await send_Notification_mail(
          receiverInfo.email,
          "Follower added!",
          `${senderInfo.userName} is following you`,
          receiverInfo.userName,
          `/user/${senderInfo._id}`
        );
        await Notification.create({
          senderInfo: senderInfo._id,
          receiver: receiverInfo._id,
          message: `${senderInfo.userName} is following you.`,
          type: "followerRequest",
          read: false,
        });
      } else {
        receiverInfo.followers.splice(
          receiverInfo.followers.indexOf(senderInfo._id),
          1
        );
        await receiverInfo.save();
        senderInfo.following.splice(
          senderInfo.following.indexOf(receiverInfo._id),
          1
        );
        await senderInfo.save();
      ...
      await send_Notification_mail(
        receiverInfo.email,
        `Message Request from ${senderInfo.userName}`,
        `${senderInfo.userName} sent a message request please check the notification section.`,
        receiverInfo.userName,
        "/conversations"
      );
      return res
        .status(200)
        .send(`Message request sent to ${receiverInfo.userName}`);
    } else {
      const receiverInfo = await User.findOne({ _id: req.body.receiverId });
      const text =
        conversationExists[0].status == "pending"
          ? `Already request sent. It is in ${conversationExists[0].status} status`
          : `Already conversation approved by ${receiverInfo.userName}`;
  ...
  try {
    ...
    if (conversationExists.length == 0) {
      ...
      if (req.body.status == "pending") {
        ...
        return res
          .status(200)
          .send(`Conversation with ${receiverInfo.userName} created`);
      }
      await Notification.create({
        senderInfo: senderInfo._id,
        receiver: receiverInfo._id,
        message: `${senderInfo.userName} has created a direct conversation with you.`,
        type: "chat",
        read: false,
      });
      return res
      ...
      const text =
        conversationExists[0].status == "pending"
          ? `Already request sent. It is in ${conversationExists[0].status} status`
          : `Already conversation exists with this user`;
      return res.status(200).send(text);
    }
  } catch (error) {
    console.log(error);
  ...
  try {
    ...
    if (conversationExists) {
      const receiverExist = await User.findOne({
        _id: conversationExists.members[1],
      });
      const senderExist = await User.findOne({
        _id: conversationExists.members[0],
      });
      if (req.body.status == "rejected") {
        await Conversation.deleteOne({ _id: req.body.conversationId });
        await send_Notification_mail(
          senderExist.email,
          `Message Update from ${receiverExist.userName}`,
          `${receiverExist.userName} has ${req.body.status} your message request and added reason: "${req.body.rejectReason}"`,
          senderExist.userName,
          "/conversations"
        );
        await send_Notification_mail(
          receiverExist.email,
          `Message Update`,
          `You have ${req.body.status} the message request sent by ${senderExist.userName}"`,
          receiverExist.userName,
          "/conversations"
        );
        await Notification.create({
          senderInfo: receiverExist._id,
          receiver: senderExist._id,
          message: `${receiverExist.userName} has ${req.body.status} your message request and added reason: "${req.body.rejectReason}"`,
          type: "chat",
          read: false,
        });
      ...
      const selectedPitch = await Pitch.findOne({
        _id: conversationExists.pitchId,
      });
      if (selectedPitch) {
  ...
  try {
    ...
    if (file !== "") {
      ...
      await Message.create({
        email,
        file: { public_id: result.public_id, secure_url: result.secure_url },
        conversationId,
        senderId,
        receiverId,
        message,
    ...
    await Conversation.updateOne(
      { _id: conversationId },
      {
        $set: { seen: "", lastMessageTo: receiverId, lastMessageText: message },
      }
    );
  ...
  try {
    ...
    if (MessageExists[0] && MessageExists[0].seen == undefined) {
      await Message.updateOne(
        { _id: MessageExists[0]._id },
        { $set: { seen: new Date() } }
      );
      await Conversation.updateOne(
        { _id: MessageExists[0].conversationId },
        { $set: { seen: "seen" } }
      );
...
exports.getFrienddetailsByConversationId = async (req, res, next) => {
  try {
    ...
    const result = await Conversation.findOne({ _id: conversationId }).populate(
      { path: "members", select: ["email", "userName", "image", "role"] }
    );
    return res.status(200).send(result);
  } catch (error) {
    console.log(error);
...
exports.chatBlock = async (req, res, next) => {
  try {
    ...
    const conversation = await Conversation.findOne({
      _id: conversationId,
      "chatBlocked.blockedBy": blockedBy,
    });
    if (conversation) {
      await Conversation.updateOne(
        { _id: conversationId },
    ...
    await Conversation.updateOne(
      { _id: conversationId },
      { $set: { chatBlocked: { userInfo: user._id, blockedBy: blockedBy } } }
    );
